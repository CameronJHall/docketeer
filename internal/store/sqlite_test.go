...
func TestSQLiteStore_CreateAndGetItem(t *testing.T) {
...
func TestSQLiteStore_UpdateItem(t *testing.T) {
	s := newTestSQLiteStore(t)
...
func TestSQLiteStore_DeleteItem(t *testing.T) {
	s := newTestSQLiteStore(t)
...
func TestSQLiteStore_ListItems(t *testing.T) {
	s := newTestSQLiteStore(t)
...
func TestSQLiteStore_AddAndListNotes(t *testing.T) {
	s := newTestSQLiteStore(t)
...
func TestSQLiteStore_CompletionsLast7Days(t *testing.T) {
	s := newTestSQLiteStore(t)
	...
	for i, item := range tasks {
		...
		switch i {
		case 1:
			_, _ = s.db.Exec("UPDATE items SET updated_at = ? WHERE id = ?", now.AddDate(0, 0, -1).Unix(), item.ID)
		case 2:
			_, _ = s.db.Exec("UPDATE items SET updated_at = ? WHERE id = ?", now.AddDate(0, 0, -6).Unix(), item.ID)
		case 3:
			_, _ = s.db.Exec("UPDATE items SET updated_at = ? WHERE id = ?", now.AddDate(0, 0, -8).Unix(), item.ID)
		}
	}
